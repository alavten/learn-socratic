...
def test_full_loop_ingest_learn_quiz_review_switch(isolated_db):
    ...
    learn_prompt = service.get_learn_context(plan_id, topic_id="t1")
    assert "Socratic" in learn_prompt["prompt_text"]
    learn_commit = service.add_interaction_record(
        plan_id,
        "learn",
    ...
    quiz_prompt = service.get_quiz_context(plan_id, topic_id="t1")
    assert "quiz" in quiz_prompt["prompt_text"].lower()
    quiz_commit = service.add_interaction_record(
        plan_id,
        "quiz",
    ...
    review_prompt = service.get_review_context(plan_id)
    assert "review" in review_prompt["prompt_text"].lower()
    review_commit = service.add_interaction_record(
        plan_id,
        "review",
