...
def test_ingest_fails_when_relation_type_is_not_supported(isolated_db):
    ...
    with pytest.raises(PayloadValidationError) as exc_info:
        service.ingest_knowledge_graph("g1", payload)
    assert exc_info.value.details["field_path"] == "$.structured_payload.relations.0.relation_type"
...
def test_ingest_fails_with_actionable_error_when_evidence_quote_missing(isolated_db):
    ...
    with pytest.raises(PayloadValidationError) as exc_info:
        service.ingest_knowledge_graph("g1", payload)
    assert exc_info.value.details["field_path"] == "$.structured_payload.evidences.0"


def test_get_learn_context_with_unknown_plan_returns_error_context(isolated_db):
    service = OrchestrationAppService()

    response = service.get_learn_context("missing-plan")
