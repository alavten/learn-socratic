...
面向「文档 → 知识图谱 → 新学 / 测验 / 复习」的苏格拉底式学习技能。由 Agent 按 `SKILL.md` 路由，并在各模式的契约文件（`references/*.md`）下执行。主模式四字：**ingest / learn / quiz / review**（另有 **shared** 用于澄清与恢复）。
...
- **图谱写入**：修改知识图谱前需你明确确认；**学习/测验/复习记录**在判分后由技能契约要求立即写入（写入失败则先恢复，不跳步）。

## 五种模式怎么选

用你的说法对照目标模式（详细规则见对应 `references/*.md`）：

| 你想做的事                                          | 模式     | 契约文件          |
| --------------------------------------------------- | -------- | ----------------- |
| 导入材料、建图、更新图谱                            | `ingest` | `references/ingest.md` |
| 讲解、带我理解                                      | `learn`  | `references/learn.md`  |
| 测验、考我、出题                                    | `quiz`   | `references/quiz.md`   |
| 复习、背诵、到期巩固                                | `review` | `references/review.md` |
| 意图模糊、说不清跟哪份图谱/资料学、或要从失败里恢复 | `shared` | `references/shared.md` |
...
- **节奏**：一轮围绕一小块内容、通常一道核心检查题；判分与进度会记下来，失败时会先补上记录再继续，避免跳步。细则见 `references/learn.md`。

### `quiz`（测验）

- **适合**：已经在某套学习计划里学过一阵，想 **做题摸底**。
- **你怎么配合**：同样基于一份学习计划；可选指定章节或主题缩小范围。
- **节奏**（两种，详见 `references/quiz.md`）：
  - **逐概念**（`per_concept`）：与「一个概念一个概念学」对齐，默认每概念 1 题；复杂概念可在同一轮多问几题，**每题判完即写一条记录**。
  - **按章节**（`per_chapter`）：与「整章/多概念一次学完」对齐，一轮出多题；收齐答案后逐题判定并写入，本轮结束前核对「题数 = 记录数」再进入讲解或下一章。
- 可说「一题一题」或「批量/一章测验」覆盖默认；落库始终是 **每道已判定题一条记录**，不靠 `shared` 事后补录。

### `review`（复习）

- **适合**：巩固 **快忘的、到期的、错题相关的** 内容。
- **你怎么配合**：基于学习计划进行；可以说想复习哪一块主题；接着上课时助手会接着上次的复习队列往下走。
- **节奏**：按队列逐个概念过；每次判定后落笔再前进，下一轮接着排。详见 `references/review.md`。

### `shared`（澄清与恢复）

- **适合**：还没选定学哪套资料、意图含糊，或上一轮中途卡住需要重来。若要 **改评分或补记录**，会把你交回 `quiz`/`learn`/`review` 逐条写入，而不是在 `shared` 里批量改库。
